...
#include "delay.h"
#include "usart.h"
float Velocity_KP=300,Velocity_KI=1,Velocity_KD=0;
float Velocity_KPA=20,Velocity_KIA=0,Velocity_KDA=50;		   //左
float Velocity_KPB=20,Velocity_KIB=0,Velocity_KDB=50;	       //右，80，60
...
{
		  if(motor_a<0)			PWMB1=7200,PWMB2=7200+motor_a;
			else 	            PWMB2=7200,PWMB1=7200-motor_a;
}
...
int PID_position (int distance,int Target)//保持距离
{
	 ...
	 PwmA=Velocity_KP*BiasA+Velocity_KI*Integral_biasA+Velocity_KD*(BiasA-Last_biasA);   //增量式PI控制器
...
{ 	
	 static int BiasA=0,PwmA=0,Last_biasA=0,Integral_biasA=0,tiaojie=0;
	 BiasA=Target-Encoder;                //计算偏差
	 Integral_biasA+=BiasA;
	 tiaojie=Velocity_KPA*BiasA+Velocity_KIA*Integral_biasA+Velocity_KDA*(BiasA-Last_biasA);   //增量式PI控制器
	 PwmA+=tiaojie;
//	 printf("%d %d %d %d \r\n",tiaojie,Velocity_KPA*BiasA,Velocity_KIA*Integral_biasA,Velocity_KDA*(BiasA-Last_biasA));	
	 Last_biasA=BiasA;	                   //保存上一次偏差 
	 return PwmA;                         //增量输出
}
