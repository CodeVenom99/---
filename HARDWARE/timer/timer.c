...
//		}
		...
		target=v_t*100.0/1.7;//理论上为130/1.7，线速度转换为编码器读数
		Encoder_Left=Read_Encoder(2);	
		Encoder_Right=Read_Encoder(3);	
		Motor_A=Incremental_PI_A(Encoder_Left,target);		
		Motor_B=Incremental_PI_B(Encoder_Right,target);	
		
//		printf("%d\r\n",Encoder_Left,Motor_B);
		Xianfu_Pwm();	
